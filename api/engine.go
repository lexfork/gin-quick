...
func (config *EngineConfig) initEngineConfig(api *API) *gin.Engine {
	...
	// logger := logruslogger.GetLogger("api-deal", true, conf) // re create logger
	// gin api handlers, used for api log info
	// e.Use(logruslogger.Logger(logger.GetLogger()))

	// gin api handlers, used for api log info
	switch api.log.(type) {
	case *logruslogger.LogrusLogger:
		logger, _ := api.log.(*logruslogger.LogrusLogger)
		logHandleFunc := middleware.LoggerMiddlewareLogrus(logger)
		e.Use(logHandleFunc)
